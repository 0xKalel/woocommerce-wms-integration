...
class WC_WMS_Product_Service implements WC_WMS_Product_Service_Interface {
    ...
    private function importSingleArticle(array $article): array {
        ...
        if ($existingProduct) {
            ...
            // Create new product using centralized ProductSyncManager
            $newProduct = $this->client->productSyncManager()->createSimpleProductFromVariant($primaryVariant, $sku);
            return [
                'action' => 'created',
                'product_id' => $newProduct->get_id(),
                'sku' => $sku
            ];
        }
    }
    ...
    /**
     * Update existing WooCommerce product with WMS article data
     */
    private function updateWooCommerceProduct(WC_Product $product, array $article, array $variant): WC_Product {
        // Update basic product data
        $product->set_name($article['name'] ?? $product->get_name());
        $product->set_description($variant['description'] ?? $product->get_description());
        ...
        // Update stock
        $stockQuantity = $variant['stock_physical'] ?? $product->get_stock_quantity();
        ...
        $this->client->logger()->info('Updated WooCommerce product', [
            'product_id' => $product->get_id(),
            'sku' => $product->get_sku(),
            'name' => $product->get_name()
        ]);
